        ...
        document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    showToast(result.error || 'Failed to create opportunity');
                }
            } catch (error) {
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                showDashboard(email);
                loadOpportunities(); // Load user's opportunities
            }, 1200);
        } else {
            showError('loginPasswordErr', result.error || 'Login failed');
            shakeForm('loginForm');
        }
    } catch (error) {
...
document.getElementById('signupForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showError('signupEmailErr', result.error || 'Signup failed');
...
document.getElementById('forgotForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showError('forgotEmailErr', result.error || 'Request failed');
...
async function loadOpportunities() {
    try {
        ...
        if (response.ok) {
            ...
            displayOpportunities([]); // Show empty state
        }
    } catch (error) {
...
function displayOpportunities(opportunities) {
    ...
    // Clear existing opportunities (remove hardcoded ones)
    grid.innerHTML = '';

    if (opportunities.length === 0) {
        // Show empty state
        grid.innerHTML = `
            <div class="empty-state" style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: var(--qf-text-light);">
                <svg viewBox="0 0 24 24" style="width: 64px; height: 64px; margin-bottom: 16px; opacity: 0.5;">
                    <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>
                    <polyline points="22 4 12 14.01 9 11.01"/>
                </svg>
                <h3 style="margin-bottom: 8px; font-weight: 500;">No Opportunities Yet</h3>
...
function createOpportunityCard(opportunity) {
    ...
    const skills = Array.isArray(opportunity.skills) ? opportunity.skills : 
                  (typeof opportunity.skills === 'string' ? opportunity.skills.split(',').map(s => s.trim()) : []);

    // Format start date
    const startDate = opportunity.start_date ? new Date(opportunity.start_date).toLocaleDateString('en-US', { 
        year: 'numeric', 
        month: 'short' 
    }) : 'TBD';

    // Applicants count
    const applicantsCount = opportunity.max_applicants ? `${opportunity.max_applicants} max applicants` : '0 applicants';

    card.innerHTML = `
        <div class="opportunity-card-header">
            <h5>${escapeHtml(opportunity.title)}</h5>
        ...
        <div class="opportunity-footer">
            ...
            <div class="opportunity-actions">
                <button class="view-course-btn" onclick="viewOpportunityDetails(${opportunity.id})" style="width: auto; padding: 8px 16px; margin-right: 8px;">View Details</button>
                <button class="edit-opportunity-btn" onclick="editOpportunity(${opportunity.id})" style="width: auto; padding: 8px 16px; margin-right: 8px; background: var(--qf-primary-light); color: var(--qf-primary);">Edit</button>
                <button class="delete-opportunity-btn" onclick="deleteOpportunity(${opportunity.id})" style="width: auto; padding: 8px 16px; background: #dc3545; color: white;">Delete</button>
            </div>
        </div>
    `;
...
async function viewOpportunityDetails(opportunityId) {
    try {
        ...
        if (response.ok) {
            ...
            openOpportunityDetails(opportunity.title, {
...
async function editOpportunity(opportunityId) {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('oppName').value = opportunity.title;
            ...
            document.getElementById('oppSkills').value = Array.isArray(opportunity.skills) ? 
                opportunity.skills.join(', ') : opportunity.skills;
            document.getElementById('oppCategory').value = opportunity.category.toLowerCase();
            ...
            // Store the opportunity ID for updating
            document.getElementById('opportunityForm').dataset.editingId = opportunityId;
...
async function deleteOpportunity(opportunityId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            loadOpportunities(); // Reload the list
        } else {
            const result = await response.json();
            showToast(result.error || 'Failed to delete opportunity');
        }
    } catch (error) {
        showToast('Network error. Please try again.');
    }
...
// Update the opportunity form submission to handle both create and edit
document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    try {
        const url = isEditing ? `/api/opportunities/${editingId}` : '/api/opportunities';
        ...
        if (response.ok) {
            ...
            showToast(result.error || `Failed to ${isEditing ? 'update' : 'create'} opportunity`);
        }
    } catch (error) {
        showToast('Network error. Please try again.');
    }
...
// Override the closeOpportunityModal function to reset form state
const originalCloseOpportunityModal = closeOpportunityModal;
closeOpportunityModal = function() {
    ...
    document.querySelector('#opportunityForm button[type="submit"]').textContent = 'Create Opportunity';
    form.reset();
    originalCloseOpportunityModal();
};

// Load opportunities when the opportunity section is shown
document.querySelector('.nav-item[data-page="opportunity"]').addEventListener('click', function() {
    setTimeout(loadOpportunities, 100); // Small delay to ensure section is visible
});